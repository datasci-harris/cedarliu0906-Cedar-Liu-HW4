...
# Create a function that takes a year as an input, generates the appropriate url
def fetch_html(year):
    url=f"https://en.wikipedia.org/wiki/List_of_American_films_of_{year}"
    response=requests.get(url)
    return response.text

# converts the relevant table into a list object
def parse_html(html, year):
    ...
    for row in rows[1:]:
        ...
        clean_rows=[]
        for cell in cells:
            clean_text=cell.text.strip().replace('/n', ' ')
            clean_rows.append(clean_text)
            
        # The way to tackle the problem of merged cell of distributor
        if len(clean_rows)==3:
            clean_rows.insert(2, last_distributor)
        else:
            last_distributor=clean_rows[2]

        rank=clean_rows[0]
        title=clean_rows[1]
        distributor=clean_rows[2]
        domestic_gross=clean_rows[3]
        movies.append([rank, title, distributor, domestic_gross, year])
...
def fetch_and_parse(year):
    ...
    movies=parse_html(html, year)
    return movies

# Write a loop that goes from 1970 to 2023
movies_list=[]
for year in range(1969, 2024):
    movies=fetch_and_parse(year)
    movies_list.extend(movies)
    
    # Use the sleep() method to ensure taking a 3 second break between requests
    sleep(3)
                
# Convert final_table into a dataframe df; save it as a CSV
df=pd.DataFrame(movies_list, columns=['Rank', 'Title', 'Distributor', 'Domestic gross', 'Year'])
moviesfile_path="E:/Working Files/UChicago MPP/24spring/PPHA 30537 2 Data and Programming for Public Policy I - Python Programming/hwk/hwk4"
movies_path=os.path.join(moviesfile_path, "movies.csv")
df.to_csv('movies.csv', index=False)
                
# Open the CSV and pick a random movie
df_movies=pd.read_csv(movies_path)

# Check if the outputs is the same as what's mentioned above
print(df_movies.head())
print(df_movies.tail())

# pcik a random movie with the "random" package
random_index=random.randint(0, len(df_movies)-1)
random_movies=df_movies.iloc[random_index]
print(f"{random_movies['Title']} ({random_movies['Year']})")
